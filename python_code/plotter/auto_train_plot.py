...
def PlotDecs(decs_to_plot, decs_to_calc_and_plot, dec_trained_params, plot_type="FER"):
    ...
    for dec_name in decs_to_calc_and_plot:
        graph_params, runs_params, script_params = dec_name()
        if 'label' not in graph_params:
    ...
    for dec_name in dec_trained_params:
        graph_params = dec_name['graph_params']
        runs_params = dec_name['runs_params']
        script_params = dec_name['script_params']
        if 'label' not in graph_params:
            label = f"{script_params['decoder_type']} ({runs_params['code_len']},{runs_params['info_len']}) iters {runs_params['iteration_num']}"
            graph_params['label'] = label
        start = time()
...
if __name__ == '__main__':
    ...
    decs_to_train_and_plot = [get_weighted_polar_1024_512_crc11, get_ensemble_polar_1024_512_crc11_iter5_decs_2, get_ensemble_polar_1024_512_crc11_iter5_decs_4, get_ensemble_polar_1024_512_crc11_iter5_decs_8]
    decs_to_plot_only = [get_polar_1024_512_crc11]
    decs_to_load_plot = []
