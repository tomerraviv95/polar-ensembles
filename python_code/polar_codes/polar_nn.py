...
def get_masks_dicts(code_len, connections, pi=None):
    ...
    for i in range(l_stages):
        mask = maskt[:, i]
        mask_resized_dict[i] = torch.masked_select(torch.arange(len(mask)), mask).long().view(code_len // 2)
        negative_mask_resized_dict[i] = torch.masked_select(torch.arange(len(mask)), ~mask).long().view(code_len // 2)
...
class IterateLeftLayer(torch.nn.Module):
    ...
    def forward(self, right, left):
        for i in reversed(range(self.num_stages)):
            ...
            left[:, i, self.mask_dict[i]] = self.left_weights[i, 0] * min_sum(left_prev1, left_prev0 + right_prev0)
            left[:, i, self.negative_mask_dict[i]] = self.left_weights[i, 1] * min_sum(left_prev1,
