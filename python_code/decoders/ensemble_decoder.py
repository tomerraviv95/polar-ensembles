...
class EnsembleDecoder(Decoder):
    ...
    def forward(self, rx: torch.Tensor, pass_all_words = False, take_crc_0=False, get_list = False, get_mask=False):
        """
        ...
        if pass_all_words: #Pass all the words through all of the decoders (for evaluation)
            ...
            for dec_id,dec in enumerate(self.decoders):
                ...
                if take_crc_0:
                    ...
                    best_output[idx] = words[idx]
                if get_list:
                    if dec_id != 0:
                        output_list.append(words)
            output = best_output

        else: #Pass only the wrongly decoded words through the designated decoder (for training)
            for dec_id,dec in enumerate(self.decoders):
                if dec_id != 0: # already decoded
                    idx = (np_dec_mask == dec_id).flatten("F")
                    words_per_decoder = np.sum(idx)
                    if words_per_decoder != 0:
                        curr_rx = rx[idx]
                        output_t, not_satisfied = dec(curr_rx)
                        words = output_t[-1]
                        output[idx] = words
        if get_mask:
